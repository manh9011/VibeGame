import { NotchSprite } from './notchSprite';
import { Resources } from '../Enjine/resources';
import { Sparkle } from './sparkle';
import type { LevelState } from './levelState';

/**
 * Represents the animation of a coin popping out of a block.
 */
export class CoinAnim extends NotchSprite {
    /** Reference to the level state. */
    World: LevelState | null;
    /** Life duration of the animation. */
    Life: number;

    constructor(world: LevelState | null, x: number, y: number) {
        super(null);
        this.World = world;
        this.Life = 10;
        this.Image = Resources.Images["map"];
        this.PicWidth = this.PicHeight = 16;
        this.X = x * 16;
        this.Y = y * 16 - 16;
        this.Xa = 0;
        this.Ya = -6;
        this.XPic = 0;
        this.YPic = 2;
    }

    /**
     * Updates the coin animation.
     */
    Move(): void {
        var x = 0, y = 0;
        if (this.Life-- < 0) {
            this.World.RemoveSprite(this);
            for (x = 0; x < 2; x++) {
                for (y = 0; y < 2; y++) {
                    this.World.AddSprite(new Sparkle(this.World, (this.X + x * 8 + Math.random() * 8) | 0, (this.Y + y * 8 + Math.random() * 8) | 0, 0, 0, 0, 2));
                }
            }
        }

        this.XPic = this.Life & 3;
        this.X += this.Xa;
        this.Y += this.Ya;
        this.Ya += 1;
    }
}
