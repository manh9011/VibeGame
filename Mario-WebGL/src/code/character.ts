...
/**
 * The main character class (Mario). Handles movement, physics, power-ups, and collisions.
 */
export class Character extends NotchSprite {
    /** Whether Mario is in Large state. */
    Large: boolean;
    /** Whether Mario has Fire power. */
    Fire: boolean;
    /** Number of collected coins. */
    Coins: number;
    /** Number of remaining lives. */
    Lives: number;
    /** String representing current level. */
    LevelString: string;
    ...
    RunTime: number;
    /** Whether Mario was on the ground in the previous frame. */
    WasOnGround: boolean;
    /** Whether Mario is currently on the ground. */
    OnGround: boolean;
    /** Whether Mario is allowed to jump. */
    MayJump: boolean;
    /** Whether Mario is ducking. */
    Ducking: boolean;
    /** Whether Mario is sliding. */
    Sliding: boolean;
    /** Jump duration timer. */
    JumpTime: number;
    ...
    WinTime: number;
    /** Timer for invulnerability after taking damage. */
    InvulnerableTime: number;
    /** Object currently being carried by Mario. */
    Carried: NotchSprite | null;
    /** Previous large state (for blinking). */
    LastLarge: boolean;
    /** Previous fire state (for blinking). */
    LastFire: boolean;
    /** New large state (for blinking). */
    NewLarge: boolean;
    /** New fire state (for blinking). */
    NewFire: boolean;
    ...
    /**
     * Initializes Mario within the given world.
     */
    ...
    /**
     * Sets Mario's power-up state.
     * @param large Whether Mario is large.
     * @param fire Whether Mario has fire flower.
     */
    ...
    /**
     * Handles the blinking animation when changing state or taking damage.
     */
    ...
    /**
     * Main update loop for Mario. Handles input, physics, and state.
     */
    Move(): void {
        if (this.WinTime > 0) {
            this.WinTime++;
        ...
        if (this.Sliding) {
            this.World.AddSprite(new Sparkle(this.World, ((this.X + Math.random() * 4 - 2) | 0) + this.Facing * 8,
                ((this.Y + Math.random() * 4) | 0) - 24, Math.random() * 2 - 1, Math.random(), 0, 1));
            this.Ya *= 0.5;
        }
    ...
    /**
     * Calculates which sprite frame to show based on state and movement.
     */
    CalcPic(): void {
        var runFrame = 0, i = 0;
        ...
        if (this.OnGround && ((this.Facing === -1 && this.Xa > 0) || (this.Facing === 1 && this.Xa < 0))) {
            ...
            if (this.Xa > 3 || this.Xa < -3) {
                for (i = 0; i < 3; i++) {
                    this.World.AddSprite(new Sparkle(this.World, (this.X + Math.random() * 8 - 4) | 0, (this.Y + Math.random() * 4) | 0, Math.random() * 2 - 1, Math.random() * -1, 0, 1));
                }
            }
        }
    ...
    /**
     * Handles sub-pixel movement and collision detection.
     */
    ...
    IsBlocking(x: number, y: number, xa: number, ya: number): boolean {
        ...
        if (((Tile.Behaviors[block & 0xff]) & Tile.PickUpable) > 0) {
            ...
            for (xx = 0; xx < 2; xx++) {
                for (yy = 0; yy < 2; yy++) {
                    this.World.AddSprite(new Sparkle(this.World, x * 16 + xx * 8 + ((Math.random() * 8) | 0), y * 16 + yy * 8 + ((Math.random() * 8) | 0), 0, 0, 0, 2));
                }
            }
        }
    ...
    /**
     * Called when Mario reaches the end of the level.
     */
    Win(): void {
    ...
    /**
     * Called when Mario collects a fire flower.
     */
    GetFlower(): void {
        if (this.DeathTime > 0 && this.World.Paused) {
            return;
    ...
    /**
     * Called when Mario collects a mushroom.
     */
    GetMushroom(): void {
        if (this.DeathTime > 0 && this.World.Paused) {
            return;
